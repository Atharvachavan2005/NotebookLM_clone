...
def initialize_pipeline():
    ...
    try:
        ...
        with st.spinner("Initializing KnowledgeCast pipeline..."):
            ...
            try:
                podcast_tts_generator = PodcastTTSGenerator() if gemini_key else None
...
def render_sources_sidebar():
    with st.sidebar:
        st.markdown('<div class="main-header">🗃️ Knowledge Context</div>', unsafe_allow_html=True)
...
def render_source_upload_dialog():
    ...
    st.markdown("""
    Sources let Resona base its responses on the information that matters most to you.  
...
def generate_podcast(selected_source: str, podcast_style: str, podcast_length: str):
    if not st.session_state.pipeline or not st.session_state.pipeline['podcast_script_generator']:
        st.error("Podcast generation not available. Please check your Gemini API key.")
        return
...
def main():
    ...
    <div style="display: flex; align-items: center; margin-bottom: 30px;">
        <h1 style="color: #ffffff; margin: 0;">🧠 KnowledgeCast: Turn your documents into a conversation. Turn your research into a show</h1>
    </div>
    """, unsafe_allow_html=True)
    ...
    <div style="text-align: center; color: #a0aec0; font-size: 12px;">
        KnowledgeCast can be inaccurate; please double check its responses.
    </div>
    """, unsafe_allow_html=True)
